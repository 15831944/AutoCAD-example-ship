...
ACRX_DXF_DEFINE_MEMBERS (
	...
	AcDbProxyEntity::kNoOperation, SHIP,
CUSTDBXAPP
...
Ship::Ship () : AcDbEntity () {
	...
	m_fHw = 1.0; m_fHeight = 1.0;
	storePoints();
}

Ship::Ship(AcGePoint3d Pa, AcGePoint3d Pf, double hw, double hz) : AcDbEntity()
{
	m_ptAft = Pa; m_ptFore = Pf; m_fHw = hw; m_fHeight = hz;
	storePoints();
}

Ship::Ship(const Ship& o)
{
	m_ptAft = o.m_ptAft; m_ptFore = o.m_ptFore; m_fL = o.m_fL;
	m_fHeight = o.m_fHeight; m_fHw = o.m_fHw;
	m_vecU = o.m_vecU; m_vecV = o.m_vecV; m_vecW = o.m_vecW;
	m_nBaseline = o.m_nBaseline; m_arBaseline = o.m_arBaseline;
	m_nBulwark = o.m_nBulwark; m_arBulwark = o.m_arBulwark;
}
...
Acad::ErrorStatus Ship::dwgOutFields (AcDbDwgFiler *pFiler) const {
	...
	if ( (es =pFiler->writeUInt32 (Ship::kCurrentVersionNumber)) != Acad::eOk )
		return (es) ;
	//----- Вывод параметров
	//.....
	...
	int i;
	if (m_nBaseline > 0)
		for (i = 0; i < m_nBaseline; i++) pFiler->writeItem(m_arBaseline.at(i));
	...
	if (m_nBulwark > 0)
		for (i = 0; i < m_nBulwark; i++) pFiler->writeItem(m_arBulwark.at(i));
